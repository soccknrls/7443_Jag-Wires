...
public class PidController {
    ...
    public double calculate(double current, double dt) {
        ...
        if (shouldClearIntegralOnErrorSignChange && !Vector2.epsilonEquals(error, Math.copySign(error, integralAccum)) && !Vector2.epsilonEquals(integralAccum, 0.0)) {
            integralAccum = 0.0;
        }
        ...
        return Vector2.clamp(constants.p * error + constants.i * integral + constants.d * derivative,
                minOutput, maxOutput);
    }
